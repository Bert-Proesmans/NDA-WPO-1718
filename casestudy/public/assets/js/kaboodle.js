function runKaboodle(window) {
  ...
  var kaboodleTag = document.getElementById("kaboodle-container");
  var src = kaboodleTag.getAttribute("src");
  var kaboodleRoot = "https://s3-us-west-2.amazonaws.com/kaboodle";
  ...
  kaboodleTag.appendChild(kaboodleModule);
  ...
  function loadItems() {
    ...
    for (var i = 0; i < numCols * numRows; i++) {
      ...
      kaboodleItemLink.onclick = function () {
        if (ga) {
          ga('send', 'event',
            'kaboodleAd', 'click', creative["href"], {
              'hitCallback':
                function () {
